...
pub const Context = struct {
    ...
    pub fn deinit(self: *Context) void {
        self.values.deinit(self.wasm_file.base.allocator);
        self.blocks.deinit(self.wasm_file.base.allocator);
        self.locals.deinit(self.wasm_file.base.allocator);
        self.* = undefined;
    }
    ...
    fn fail(self: *Context, comptime fmt: []const u8, args: anytype) InnerError {
        ...
        self.err_msg = try Module.ErrorMsg.create(self.wasm_file.base.allocator, src_loc, fmt, args);
    ...
    fn allocLocal(self: *Context, ty: Type) InnerError!WValue {
        ...
        switch (ty.zigTypeTag()) {
            .Struct => {
                ...
                try self.locals.ensureCapacity(self.wasm_file.base.allocator, self.locals.items.len + fields_len);
            ...
            .ErrorUnion => {
                ...
                try self.locals.ensureCapacity(self.wasm_file.base.allocator, self.locals.items.len + 2);
                ...
                try self.locals.append(self.wasm_file.base.allocator, valtype);
                self.local_index += 1;
    ...
    fn genFunctype(self: *Context) InnerError!void {
        ...
        if (ty.fnParamLen() != 0) {
            const params = try self.wasm_file.base.allocator.alloc(Type, ty.fnParamLen());
            defer self.wasm_file.base.allocator.free(params);
    ...
    fn genBody(self: *Context, body: []const Air.Inst.Index) InnerError!void {
        for (body) |inst| {
            ...
            try self.values.putNoClobber(self.wasm_file.base.allocator, inst, result);
        }
    }
    ...
    fn airCall(self: *Context, inst: Air.Inst.Index) InnerError!WValue {
        ...
        // in link.Wasm.flush().
        try self.decl.fn_link.wasm.idx_refs.append(self.wasm_file.base.allocator, .{
    ...
    fn airStore(self: *Context, inst: Air.Inst.Index) InnerError!WValue {
        ...
        switch (lhs) {
            .multi_value => |multi_value| switch (rhs) {
                ...
                .multi_value => self.values.put(self.wasm_file.base.allocator, Air.refToIndex(bin_op.lhs).?, rhs) catch unreachable, // Instruction does not dominate all uses!
                .constant, .none => {
        ...
        switch (ty.zigTypeTag()) {
            ...
            .Pointer => {
                if (value.castTag(.decl_ref)) |payload| {
                    ...
                    const got_addr = self.wasm_file.getDeclOffsetTableVAddr(decl, ptr_width);
                    try writer.writeByte(wasm.opcode(.i32_const));
                    try leb.writeULEB128(writer, got_addr);
    ...
    fn airBlock(self: *Context, inst: Air.Inst.Index) InnerError!WValue {
        ...
        try self.blocks.putNoClobber(self.wasm_file.base.allocator, inst, self.block_depth);
